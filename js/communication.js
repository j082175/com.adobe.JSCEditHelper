        ...
        try {
            ...
            if (typeof data !== "string") {
                ...
                try {
                    ...
                    window.JSCErrorHandler.handleCommunicationError('JSX_ERROR', "데이터 처리 오류: 서버에서 잘못된 형식의 데이터를 받았습니다.", { originalError: e.message });
                    return;
                }
            }
            // [object Object] 오류 검사
            if (data === "[object Object]") {
                window.JSCErrorHandler.handleCommunicationError('JSX_ERROR', "데이터 처리 오류: 서버 응답 형식에 문제가 있습니다.", { receivedData: data });
                return;
            }
            // Try parsing JSON
            var resultData = window.JSCUtils.safeJSONParse(data);
            if (!resultData) {
                window.JSCUtils.logWarn("Failed to parse data as JSON, treating as string");
                window.JSCUIManager.updateStatus(data, false);
                return;
            }
            // Save debug info
            if (resultData.debug) {
            ...
            if (resultData.clipsMoved !== undefined || resultData.gapsRemoved !== undefined) {
                window.JSCUIManager.updateMagnetStatus(resultData.success || false, resultData.clipsMoved, resultData.gapsRemoved);
            }
        }
        catch (e) {
            window.JSCErrorHandler.handleError(window.JSCErrorHandler.createError(window.JSCErrorHandler.ERROR_TYPES.COMMUNICATION, 'EVENT_ERROR', "처리 중 오류가 발생했습니다. 다시 시도해주세요.", { originalError: e.message, stack: e.stack }));
        }
    }
    // FileListEvent 처리
    function handleFileListEvent(event) {
        ...
        try {
            ...
            if (typeof eventDataString === "string") {
                ...
                var parsed = window.JSCUtils.safeJSONParse(eventDataString);
                if (!parsed) {
